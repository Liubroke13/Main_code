...
\section{Wave equations from Maxwell's equations in free space}
 \label{ch:Maxwell} 
In the electrodynamics framework, Maxwell’s equations in vacuum can be written as
\begin{align} 
...
Begin with the Lorenz gauge, we assume a \textbf{monochromatic wave} ansatz,$\boldsymbol{A}^L(\boldsymbol{r}, t)=\text{Re}\left[\boldsymbol{A}^L(\boldsymbol{r}) e^{-i\omega t}\right]$, which transforms Eq.~(\ref{Lwave2}) into the vector Helmholtz equation
\begin{equation} 
